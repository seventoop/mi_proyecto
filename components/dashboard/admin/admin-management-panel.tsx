            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-[#0A0A0C] rounded-2xl p-5 border border-slate-200 dark:border-white/[0.06]">
                    <div className="flex justify-between items-start mb-2">
                        <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Volumen Global</p>
                        <Globe className="w-4 h-4 text-brand-500" />
                    </div>
                    <p className="text-2xl font-black text-slate-900 dark:text-white">
                        ${financials.globalVolume.toLocaleString()}
                    </p>
                    <p className="text-xs text-slate-500 font-bold mt-1 uppercase tracking-widest">Transaccionado en plataforma</p>
                </div>

                <div className="bg-white dark:bg-[#0A0A0C] rounded-2xl p-5 border border-slate-200 dark:border-white/[0.06]">
                    <div className="flex justify-between items-start mb-2">
                        <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Recaudación Fee</p>
                        <CreditCard className="w-4 h-4 text-blue-500" />
                    </div>
                    <p className="text-2xl font-black text-slate-900 dark:text-white">
                        ${financials.platformRevenue.toLocaleString()}
                    </p>
                    <p className="text-xs text-emerald-500 font-black mt-1 uppercase tracking-widest">Ingresos operativos netos</p>
                </div>

                <div className="bg-white dark:bg-[#0A0A0C] rounded-2xl p-5 border border-slate-200 dark:border-white/[0.06]">
                    <div className="flex justify-between items-start mb-2">
                        <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Monto en Escrow</p>
                        <ShieldAlert className="w-4 h-4 text-emerald-500" />
                    </div>
                    <p className="text-2xl font-black text-slate-900 dark:text-white">
                        ${financials.totalEscrow.toLocaleString()}
                    </p>
                    <p className="text-xs text-slate-500 font-bold mt-1 uppercase tracking-widest">Capital bajo custodia</p>
                </div>

                <div className="bg-white dark:bg-[#0A0A0C] rounded-2xl p-5 border border-slate-200 dark:border-white/[0.06]">
                    <div className="flex justify-between items-start mb-2">
                        <p className="text-xs font-black text-slate-500 uppercase tracking-widest">Salud del Sistema</p>
                    ...
                    <p className="text-xs text-slate-500 font-bold mt-1 uppercase tracking-widest">Todos los servicios activos</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="divide-y divide-slate-100 dark:divide-white/[0.04]">
                            {queues.kyc.length > 0 ? (
                                    ...
                                    <Link key={item.id} href={`/dashboard/admin/kyc/${item.id}`} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">{item.email}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <span className="text-xs font-black text-amber-500 bg-amber-500/10 px-2.5 py-1 rounded-md uppercase tracking-widest">
                                ...
                                <div className="p-8 text-center">
                                    ...
                                    <p className="text-xs text-slate-500 font-black uppercase tracking-widest">No hay verificaciones pendientes</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#0A0A0C] border border-slate-200 dark:border-white/[0.06] rounded-2xl overflow-hidden shadow-none">
                        ...
                        <div className="divide-y divide-slate-100 dark:divide-white/[0.04]">
                            {queues.projects.length > 0 ? (
                                    ...
                                    <Link key={project.id} href={`/dashboard/admin/proyectos/${project.id}`} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Documentación pendiente</p>
                                            </div>
                                        </div>
                                ...
                                <div className="p-8 text-center">
                                    <CheckCircle2 className="w-8 h-8 text-emerald-500 mx-auto mb-2 opacity-20" />
                                    <p className="text-xs text-slate-500 font-black uppercase tracking-widest">Todos los proyectos están al día</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-[#0A0A0C] border border-slate-200 dark:border-white/[0.06] rounded-2xl p-5">
                        ...
                        <div className="space-y-4">
                            {recentUsers.map((user) => (
                                <div key={user.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-white/[0.02] rounded-xl border border-slate-100 dark:border-white/[0.06]">
                                    <div className="w-8 h-8 rounded-full bg-brand-500/10 flex items-center justify-center text-brand-500 font-black text-xs">
                        ...
                        <button className="w-full mt-4 py-2 text-xs font-black text-brand-500 uppercase tracking-widest hover:text-brand-400 transition-colors">
                    ...
                    <div className="bg-[#0A0A0C] border border-white/[0.06] p-6 rounded-2xl relative overflow-hidden group">
                        ...
                        <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-1 relative z-10">Plusvalía Estimada</h3>
                        <p className="text-3xl font-black text-white relative z-10">$24.8M</p>
                        <p className="text-xs font-bold text-slate-500 mt-2 leading-tight uppercase tracking-widest relative z-10">
