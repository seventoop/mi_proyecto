    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <p className="text-slate-500 dark:text-white/40 text-[11px] font-black uppercase tracking-widest">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {newProjectPath && (
                        <Link
                            href={newProjectPath}
                            className="px-5 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-black text-[11px] uppercase tracking-wider transition-all flex items-center gap-2 shadow-lg shadow-brand-500/10 active:scale-95"
                        >
            ...
            <Link href={href} className="block relative h-48 bg-slate-100 dark:bg-white/[0.04] flex-shrink-0 overflow-hidden border-b border-slate-100 dark:border-white/[0.06]">
                ...
                {estado && (
                    <div className="absolute top-4 right-4">
                        <span className={cn("text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg backdrop-blur-md shadow-sm border", estado.class)}>
                            {estado.label}
                        </span>
                    </div>
                )}
                ...
                {p.isDemo && (
                    <div className="absolute top-4 left-4">
                        <span className="text-[9px] font-black uppercase tracking-widest text-brand-500 bg-black/80 backdrop-blur-md px-2.5 py-1 rounded-lg border border-brand-500/30">
                            DEMO 48h
                        </span>
                    </div>
                )}
            ...
            <div className="flex flex-col flex-1 p-5 gap-4">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                        {p.ubicacion && (
                            <span className="text-[11px] font-bold text-slate-500 dark:text-white/40 flex items-center gap-1.5 uppercase tracking-tight">
                ...
                {total > 0 && (
                    <div className="space-y-3 pt-1">
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-white/30">Ventas</span>
                            <span className="text-[11px] font-black text-slate-900 dark:text-brand-400 italic">{pct}%</span>
                        </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-white/[0.06] mt-auto">
                    <Link
                        href={href}
                        className="px-4 py-2 rounded-lg text-[11px] font-black uppercase tracking-wider bg-brand-500/10 text-brand-500 hover:bg-brand-500 hover:text-white transition-all duration-300"
                    >
                        Gestionar
                    </Link>
...
function StatPill({ label, value, color }: { label: string; value: number; color: string }) {
    return (
        <div className="text-center bg-slate-50 dark:bg-white/[0.04] rounded-xl py-2 px-1 border border-slate-100 dark:border-white/[0.06]">
            ...
            <p className="text-[9px] font-bold text-slate-400 dark:text-white/30 uppercase tracking-tighter mt-1">{label}</p>
        </div>
    );
}
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-slate-100 dark:border-white/[0.06] bg-slate-50 dark:bg-white/[0.01]">
                            <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">Proyecto</th>
                            <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">Tipo</th>
                            <th className="text-left px-5 py-4 text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">Estado</th>
                            <th className="text-center px-5 py-4 text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">Total</th>
                            <th className="text-center px-5 py-4 text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">Disp.</th>
                            <th className="text-center px-5 py-4 text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">Res.</th>
                            <th className="text-center px-5 py-4 text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">Vend.</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {projects.map((p) => (
                            <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                                <td className="px-5 py-4">
                                    <Link href={`${projectBasePath}/${p.id}`} className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            {p.ubicacion && (
                                                <p className="text-[10px] font-bold text-slate-400 dark:text-white/20 flex items-center gap-1.5 uppercase tracking-tight mt-0.5">
                                ...
                                <td className="px-5 py-4">
                                    <span className={cn("text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-tighter border shadow-sm", estadoConfig[p.estado]?.class || "bg-slate-500/10 text-slate-400 border-slate-500/20")}>
