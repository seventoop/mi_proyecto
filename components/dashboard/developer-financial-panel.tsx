        ...
        <div className="space-y-4">
                ...
                <div className="flex items-center justify-between gap-4 px-4 py-3 bg-rose-500/5 dark:bg-rose-500/10 border border-rose-500/20 rounded-xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] text-rose-600 dark:text-rose-500/80 font-medium">Completá tu KYC para publicar proyectos.</p>
                        </div>
                    </div>
                    <button
                        disabled
                        className="shrink-0 px-3 py-1.5 bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-white/20 rounded-lg text-[11px] font-bold uppercase tracking-wider cursor-not-allowed flex items-center gap-1.5"
                    >
                ...
                {financialCards.map((card) => (
                    <div key={card.label} className="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-4">
                        <div className="flex items-start justify-between mb-3">
                            <p className="text-[10px] font-black text-slate-500 dark:text-white/40 uppercase tracking-widest leading-tight">{card.label}</p>
                        ...
                        {card.note && <p className="text-[10px] text-slate-400 dark:text-white/30 font-medium mt-2">{card.note}</p>}
                    </div>
                ))}
            </div>
            ...
            {projectStats.length > 0 && (
                <div className="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] overflow-hidden">
                    <div className="px-5 py-3.5 border-b border-slate-100 dark:border-white/[0.06] flex items-center justify-between">
                        <p className="text-[11px] font-black text-slate-700 dark:text-zinc-200 uppercase tracking-widest">Salud de Proyectos</p>
                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 dark:bg-white/[0.03] rounded-lg border border-slate-200 dark:border-white/[0.06]">
                            ...
                            <span className="text-[10px] text-slate-400 dark:text-white/30 font-medium">Filtrar...</span>
                        </div>
                    </div>
                            ...
                            <thead>
                                <tr className="bg-slate-50 dark:bg-white/[0.01] border-b border-slate-100 dark:border-white/[0.06]">
                                    {["Proyecto", "Etapa", "Leads", "Conv. %", "Demanda", "Recaudado", ""].map((h) => (
                                        <th key={h} className="px-4 py-3 text-[9px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest">{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 dark:divide-white/[0.04]">
                                {projectStats.map((project) => (
                                    <tr key={project.id} className="group hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={cn(
                                                "text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-tighter border",
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-[10px] font-bold text-slate-500 dark:text-white/40">{project.nivelDemanda}</span>
                                            </div>
                                        </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-5">
                    <h2 className="text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-widest mb-4 flex items-center gap-2">
                    ...
                    <div className="space-y-2.5">
                        {projectStats.filter(p => p.proximoHito).slice(0, 2).map((p) => (
                            <div key={p.id} className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/[0.06]">
                                <div>
                                    <p className="text-[11px] font-bold text-brand-500 dark:text-brand-400 uppercase tracking-tight">{p.proximoHito.titulo}</p>
                                    <p className="text-[10px] text-slate-400 dark:text-white/30 font-medium">{p.nombre}</p>
                                </div>
                ...
                <div className="rounded-xl border border-brand-500/20 bg-gradient-to-br from-brand-500/[0.07] to-brand-500/[0.03] p-5 flex flex-col justify-between">
                    <div>
                        <p className="text-[10px] font-black text-brand-600 dark:text-brand-400 uppercase tracking-widest mb-2">Optimización Sugerida</p>
                    ...
                    {isKycApproved && (
                        <Link
                            ...
                            className="mt-5 flex items-center justify-center gap-2 py-2.5 bg-brand-500 hover:bg-brand-600 text-white rounded-lg text-[11px] font-black uppercase tracking-wider transition-colors shadow-md shadow-brand-500/20"
                        >
                            <PlusCircle className="w-3.5 h-3.5" />
