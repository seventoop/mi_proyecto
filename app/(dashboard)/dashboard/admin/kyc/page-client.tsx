...
export default function AdminKYCPage() {
    ...
    const handleAction = async (userId: string, status: "VERIFICADO" | "RECHAZADO", reason?: string) => {
    ...
    const confirmReject = async () => {
        ...
        await handleAction(selectedUserId, "RECHAZADO", rejectReason);
        ...
        <div className="p-6 max-w-[1600px] mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Revisa la documentación de los usuarios para aprobar sus perfiles.</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-slate-200 dark:border-slate-800">
                    ...
                    <p className="text-slate-500 mt-2">Todos los usuarios están verificados o no han enviado documentación.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {users.map((user) => (
                        <div key={user.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden transition-all">
                            ...
                            {expandedUser === user.id && (
                                <div className="p-4 pt-0 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 mt-4">Documentación presentada</h4>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                                        {user.documentacion && user.documentacion.length > 0 ? (
                                            user.documentacion.map((doc: any) => (
                                                <div
                                                    key={doc.id}
                                                    onClick={(e) => { e.stopPropagation(); setPreviewDoc(doc.archivoUrl); }}
                                            ...
                                            <p className="text-sm text-slate-500 col-span-full italic">No hay documentos cargados.</p>
                                        )}
                                    </div>

                                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); openRejectModal(user.id); }}
            ...
            <Dialog open={rejectModalOpen} onOpenChange={setRejectModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Indica el motivo por el cual la documentación no es válida. El usuario recibirá una notificación.
                        </DialogDescription>
                    </DialogHeader>
                    <textarea
                        value={rejectReason}
                    ...
                    />
                    <DialogFooter>
                        <button onClick={() => setRejectModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-500 hover:text-slate-700">Cancelar</button>
                        <button onClick={confirmReject} disabled={!rejectReason} className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-lg text-sm font-bold disabled:opacity-50">Confirmar Rechazo</button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={!!previewDoc} onOpenChange={(open) => !open && setPreviewDoc(null)}>
                <DialogContent className="max-w-4xl h-[90vh] p-0 overflow-hidden bg-black/90 border-none">
                    <div className="w-full h-full flex items-center justify-center relative">
                        ...
                        {previewDoc && (
                            <img src={previewDoc} alt="Documento" className="max-w-full max-h-full object-contain" />
                        )}
                    </div>
                </DialogContent>
            </Dialog>
