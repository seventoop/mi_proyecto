...
import { TrendingUp, Wallet, ShieldCheck, BarChart3, ArrowUpRight, Clock, MapPin, Building2, Check } from "lucide-react";
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-1">
                        Gestión de portafolio y oportunidades de fondeo mayorista.
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    {kycStatus !== "VERIFICADO" && (
                        <Link href="/dashboard/inversor/mi-perfil/kyc" className="px-4 py-2 bg-amber-500/10 text-amber-600 rounded-xl text-sm font-bold flex items-center gap-2 border border-amber-500/20 hover:bg-amber-500/20 transition-colors animate-pulse">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                            Mi Portafolio
                            <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full text-xs font-bold text-slate-600 dark:text-slate-500">{inversiones.length}</span>
                        </h2>
                    </div>
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex gap-4">
                                        <div className="w-16 h-16 rounded-xl overflow-hidden bg-slate-100 shrink-0 relative">
                                            <Image
                                                src={inv.proyecto.imagenPortada || ""}
                                                alt={inv.proyecto.nombre}
                                                fill
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-2 flex items-center gap-3">
                                                <span className={cn(
                                                    ...
                                                    inv.estado === "ESCROW" ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" : "bg-brand-500/10 text-brand-600 border border-brand-500/20"
                                                )}>
                                                    {inv.estado}
                                                </span>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-emerald-500 font-bold mt-1">Valor Actual: ${(inv.m2Comprados * (inv.proyecto.precioM2Mercado || inv.precioM2Aplicado)).toLocaleString()}</div>
                                    </div>
                                </div>
                            </div>
                        ))}
