...
import { Wallet, ArrowUpRight, ArrowDownLeft, Plus, History, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import DepositModal from "@/components/dashboard/inversor/deposit-modal";

export default function PortafolioWalletPage() {
    ...
    useEffect(() => { loadData(); }, [session?.user]);
        ...
        <div className="p-8 space-y-8 max-w-5xl mx-auto">
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2 text-xl font-bold text-slate-900 dark:text-white">
                    <History className="w-6 h-6 text-brand-500" /> Historial de Transacciones
                </div>
                <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                        {data.transacciones.length > 0 ? data.transacciones.map((t) => (
                            <div key={t.id} className="p-5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                <div className="flex items-center gap-4">
                                    <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center", t.monto > 0 ? "bg-emerald-500/10 text-emerald-600" : "bg-brand-500/10 text-brand-600")}>
            ...
            {showDeposit && <DepositModal onClose={() => { setShowDeposit(false); loadData(); }} />}
        </div>
    );
}
